...
#define BUF_VALID(b) ((b) != NULL && (b)->data != NULL && (b)->read <= (b)->write && (b)->write <= (b)->capacity)
...
void buf_set_error(buf_t *b, util_err_t error, const char *field_name) {
    if(b == NULL) { return; }
    b->error = error;
    b->failed_field = (char *)field_name; /* cast away the const, yuck!*/
}
...
util_err_t buf_get_error(buf_t *b) {
    if(b == NULL) { return UTIL_EINVAL; }
    return b->error;
}

char *buf_get_failed_field(buf_t *b) {
...
bool buf_cut(buf_t *b, size_t offset, size_t n) {
    ...
    if(remaining > 0) { memmove(&b->data[abs_offset], &b->data[abs_offset + n], remaining); }

    b->write -= n;
    return true;
}

bool buf_splice_buf(buf_t *dest, size_t dest_offset, buf_t *src, size_t src_offset, size_t n) {
    ...
    if(shift_size > 0) { memmove(&dest->data[abs_dest_offset + n], &dest->data[abs_dest_offset], shift_size); }

    /* Copy data in */
    memcpy(&dest->data[abs_dest_offset], &src->data[abs_src_offset], n);

    dest->write += n;
    return true;
}

bool buf_splice_bytes(buf_t *b, size_t offset, uint8_t *data, size_t n) {
    if(!BUF_VALID(b) || b->error != UTIL_OK) { return false; }
    if(data == NULL && n > 0) {
    ...
    if(shift_size > 0) { memmove(&b->data[abs_offset + n], &b->data[abs_offset], shift_size); }

    /* Copy data in */
    if(n > 0) { memcpy(&b->data[abs_offset], data, n); }
...
bool buf_reserve_read(buf_t *src, size_t reservation_size, buf_t *dest) {
    if(!BUF_VALID(src) || src->error != UTIL_OK) { return false; }
    if(dest == NULL) {
        buf_set_error(src, UTIL_ENULL, "destination is null");
        return false;
    }

    if(reservation_size == 0) {
        buf_set_error(src, UTIL_EINVAL, "reservation size is zero");
        return false;
    }

    if(src->read + reservation_size > src->write) {
        buf_set_error(src, UTIL_EBOUNDS, "reservation size too large");
        return false;
    }
    ...
    /* this marks the whole reserved region as written */
    dest->write = src->read + reservation_size;

    /* bump past the reservation */
    src->read = src->read + reservation_size;

    return true;
}


bool buf_reserve_write(buf_t *src, size_t reservation_size, buf_t *dest) {
    if(!BUF_VALID(src) || src->error != UTIL_OK) { return false; }
    if(dest == NULL) {
        buf_set_error(src, UTIL_ENULL, "destination is null");
        return false;
    ...
    /* Check if reservation size is valid */
    if(src->write + reservation_size > src->capacity) {
        buf_set_error(src, UTIL_EBOUNDS, "reservation size too large");
        return false;
    }
    ...
    /* this marks the whole reserved region as unwritten */
    dest->write = src->write;

    /* bump past the reservation */
    src->write = src->write + reservation_size;
...
bool buf_compact(buf_t *b) {
    ...
    if(b->read > 0 && unread > 0) { memmove(&b->data[0], &b->data[b->read], unread); }
...
size_t buf_read_pos(buf_t *b) {
    if(b == NULL) { return 0; }
    return b->read;
}

uint8_t *buf_read_ptr(buf_t *b) {
...
bool buf_read_u16_be(buf_t *b, const char *field_name, uint16_t *out) {
    ...
    *out = (uint16_t)(((uint16_t)b->data[b->read + 0] << 8)) | (uint16_t)(((uint16_t)b->data[b->read + 1] << 0));
    b->read += 2;
    return true;
}
...
bool buf_read_u16_le(buf_t *b, const char *field_name, uint16_t *out) {
    ...
    *out = (uint16_t)(((uint16_t)b->data[b->read + 0] << 0)) | (uint16_t)(((uint16_t)b->data[b->read + 1] << 8));
    b->read += 2;
    return true;
}
...
bool buf_read_u32_be(buf_t *b, const char *field_name, uint32_t *out) {
    ...
    *out = ((uint32_t)b->data[b->read + 0] << 24) | ((uint32_t)b->data[b->read + 1] << 16) | ((uint32_t)b->data[b->read + 2] << 8)
...
bool buf_read_u32_le(buf_t *b, const char *field_name, uint32_t *out) {
    ...
    *out = ((uint32_t)b->data[b->read + 0] << 0) | ((uint32_t)b->data[b->read + 1] << 8) | ((uint32_t)b->data[b->read + 2] << 16)
...
bool buf_read_u64_be(buf_t *b, const char *field_name, uint64_t *out) {
    ...
    *out = ((uint64_t)b->data[b->read + 0] << 56) | ((uint64_t)b->data[b->read + 1] << 48)
           | ((uint64_t)b->data[b->read + 2] << 40) | ((uint64_t)b->data[b->read + 3] << 32)
           | ((uint64_t)b->data[b->read + 4] << 24) | ((uint64_t)b->data[b->read + 5] << 16)
           | ((uint64_t)b->data[b->read + 6] << 8) | ((uint64_t)b->data[b->read + 7] << 0);
    b->read += 8;
    return true;
}
...
bool buf_read_u64_le(buf_t *b, const char *field_name, uint64_t *out) {
    ...
    *out = ((uint64_t)b->data[b->read + 0] << 0) | ((uint64_t)b->data[b->read + 1] << 8) | ((uint64_t)b->data[b->read + 2] << 16)
           | ((uint64_t)b->data[b->read + 3] << 24) | ((uint64_t)b->data[b->read + 4] << 32)
           | ((uint64_t)b->data[b->read + 5] << 40) | ((uint64_t)b->data[b->read + 6] << 48)
...
bool buf_read_bytes(buf_t *b, const char *field_name, uint8_t *out, size_t len) {
    ...
    if(len > 0) { memcpy(out, &b->data[b->read], len); }
    b->read += len;
    return true;
}
...
bool buf_write_bytes(buf_t *b, const char *field_name, const uint8_t *data, size_t len) {
    ...
    if(len > 0) { memcpy(&b->data[b->write], data, len); }
    b->write += len;
    return true;
}
