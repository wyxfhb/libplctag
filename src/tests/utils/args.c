 ...
 */
static int find_flag_index(args_flag_def_t *flags, size_t flags_count, char *flag_name) {
    if(!flag_name) { return -1; }

    for(size_t i = 0; i < flags_count; i++) {
        if(strcmp(flags[i].name, flag_name) == 0) { return (int)i; }
    }
    return -1;
}
 ...
 */
static bool parse_int(char *str, int64_t *out, bool allow_hex) {
    ...
    if(allow_hex && str[0] == '0' && (str[1] == 'x' || str[1] == 'X')) { base = 16; }
 ...
 */
static bool parse_float(char *str, double *out) {
    if(!str || !out) { return false; }
...
static bool parse_value(args_value_t *val, char *str, args_flag_def_t *flag_def) {
    if(!val || !flag_def) { return false; }

    val->type = flag_def->type;
    val->present = true;

    switch(flag_def->type) {
        case ARGS_TYPE_STRING: val->value.string_val = str; return true;

        case ARGS_TYPE_INT:
            if(!parse_int(str, &val->value.int_val, false)) { return false; }
            return true;

        case ARGS_TYPE_INT_HEX:
            if(!parse_int(str, &val->value.int_val, true)) { return false; }
            return true;
        ...
        case ARGS_TYPE_BOOL: {
            ...
            // False variants
            if(strcasecmp(str, "false") == 0 || strcasecmp(str, "0") == 0 || strcasecmp(str, "no") == 0
        ...
        case ARGS_TYPE_FLOAT:
            if(!parse_float(str, &val->value.float_val)) { return false; }
            return true;
...
util_err_t args_parse(int argc, char *argv[], args_flag_def_t *flags, size_t flags_count, args_result_t *result) {
    ...
    for(size_t i = 0; i < flags_count; i++) {
        if(flags[i].repeat == ARGS_MULTIPLE) {
            result->repeat_groups[i].values = malloc(sizeof(args_value_t) * ARGS_MAX_REPETITIONS);
    ...
    for(int i = 1; i < argc; i++) {
        ...
        pdlog(LOG_MODULE_ARGS, LOG_LEVEL_DETAIL, "args_parse: flag_name=%s value=%s", flag_name,
        ...
        args_flag_def_t *flag_def = &flags[flag_index];
        pdlog(LOG_MODULE_ARGS, LOG_LEVEL_DETAIL, "args_parse: found flag at index %d, debug_name=%s", flag_index, flag_def->name);

        // Check for duplicate if ARGS_ONCE
        if(flag_def->repeat == ARGS_ONCE && result->values[flag_index].present) {
            pdlog(LOG_MODULE_ARGS, LOG_LEVEL_ERROR, "args_parse: flag '%s' appears multiple times but is ARGS_ONCE", flag_name);
        ...
        pdlog(LOG_MODULE_ARGS, LOG_LEVEL_DETAIL, "args_parse: parsed value successfully for %s", flag_def->name);
        ...
        if(flags[i].repeat == ARGS_ONCE) {
            ...
            if(!result->values[i].present) {
                ...
                if(flags[i].default_value.has_default) {
                    ...
                    memcpy(&result->values[i].value, &flags[i].default_value.value, sizeof(result->values[i].value));
...
static int get_flag_index_by_name(args_result_t *result, char *flag_name) {
    if(!result || !flag_name) { return -1; }
...
args_value_t args_get_value(args_result_t *result, char *flag_name) {
    ...
    if(!result || !flag_name) { return empty; }
...
char *args_get_string(args_result_t *result, char *flag_name) {
    ...
    if(!val.present || val.type != ARGS_TYPE_STRING) { return NULL; }
...
int64_t args_get_int(args_result_t *result, char *flag_name) {
    ...
    if(val.type != ARGS_TYPE_INT && val.type != ARGS_TYPE_INT_HEX) { return INT64_MIN; }
    return val.value.int_val;
}

bool args_get_bool(args_result_t *result, char *flag_name) {
    args_value_t val = args_get_value(result, flag_name);
    if(!val.present || val.type != ARGS_TYPE_BOOL) { return false; }
    return val.value.bool_val;
}

double args_get_float(args_result_t *result, char *flag_name) {
    args_value_t val = args_get_value(result, flag_name);
    if(!val.present || val.type != ARGS_TYPE_FLOAT) { return NAN; }
...
args_value_t args_get_at(args_result_t *result, char *flag_name, size_t index) {
    ...
    if(index >= repeated.count) { return empty; }
...
void args_free(args_result_t *result) {
    ...
    if(result->repeat_groups) {
        for(size_t i = 0; i < result->flags_count; i++) {
            if(result->repeat_groups[i].values) { free(result->repeat_groups[i].values); }
        }
...
args_flag_def_t *args_get_flag_def(args_flag_def_t *flags, size_t flags_count, char *flag_name) {
    if(!flags || !flag_name) { return NULL; }
...
//     if(!flag_def) { return NULL; }
//     return flag_def->debug_name;
// }

void args_print_help(char *program_name, args_flag_def_t *flags, size_t flags_count) {
    if(!program_name || !flags) { return; }

    printf("Usage: %s [OPTIONS]\n\n", program_name);
    printf("Options:\n");

    for(size_t i = 0; i < flags_count; i++) {
        args_flag_def_t *flag = &flags[i];
        char *req = (flag->required == ARGS_REQUIRED) ? " (required)" : "";
        char *rep = (flag->repeat == ARGS_MULTIPLE) ? " (repeatable)" : "";
        char *type_str = "";
        ...
        printf("  --%s%s %-20s [%s]%s%s\n", flag->name, type_str, flag->description ? flag->description : "", flag->name, req,
               rep);
    }
}

void args_print_flags(args_flag_def_t *flags, size_t flags_count) {
    if(!flags || flags_count == 0) { return; }

    pdlog(LOG_MODULE_ARGS, LOG_LEVEL_INFO, "args_print_flags: enter");

    for(size_t i = 0; i < flags_count; i++) {
        args_flag_def_t *flag = &flags[i];
        char *required_str = (flag->required == ARGS_REQUIRED) ? "required" : "optional";
        char *multiple_str = (flag->repeat == ARGS_MULTIPLE) ? "multiple" : "single";
        char *description = flag->description ? flag->description : "";
