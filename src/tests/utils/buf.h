 ...
 *
 * This reserves a portion of the buffer starting at the read index
 * and spanning reservation_size bytes.  The passed dest buffer will
 * be set up to reference the reserved space.   The write index will be
 * moved to start after the reserved portion.
 *
 * If there is not enough space, the function will return an error.
 *
...
/**
 * @brief Reserve write space in the buffer.
 *
 * This reserves a portion of the buffer starting at the write index
 * and spanning reservation_size bytes.  The passed dest buffer will
 * be set up to reference the reserved space. The write index will be moved
 * to after the reservation.
 *
 * If there is not enough space, the function will return an error.
 *
 * This is used to reserve space for headers that will be parsed separately.
 *
 * @param src Pointer to the source buffer.
 * @param reservation_size Size of the reservation.
 * @param dest Pointer to the destination buffer to receive the reservation.
 ...
 */
static inline buf_t buf_checkpoint(buf_t *b) { return *b; }
 ...
 */
static inline void buf_restore(buf_t *b, buf_t checkpoint) { *b = checkpoint; }
